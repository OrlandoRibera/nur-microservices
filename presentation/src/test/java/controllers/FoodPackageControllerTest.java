...
class FoodPackageControllerTest {
  ...
  @BeforeEach
  void setup() {
    mockMvc = MockMvcBuilders.standaloneSetup(foodPackageController).build();
  ...
  void createPackage() throws Exception {
    FoodPackageDTO responseDTO = new FoodPackageDTO(
  ...
  void createFood() throws Exception {
    ...
    mockMvc.perform(post("/api/catering/createFoodInPackage")
        .contentType(MediaType.APPLICATION_JSON)
        .content(jsonObject.toString()))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.id").exists())
  ...
  void updateFoodStatus() throws Exception {
    ...
    mockMvc.perform(post("/api/catering/updateFoodStatus")
        .contentType(MediaType.APPLICATION_JSON)
        .content(jsonObject.toString()))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.foodId").exists())
  ...
  void packFoodPackage() throws Exception {
    ...
    mockMvc.perform(post("/api/catering/packFoodPackage")
        .contentType(MediaType.APPLICATION_JSON)
        .content(jsonObject.toString()))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.id").exists())
  ...
  void dispatchFoodPackage() throws Exception {
    ...
    mockMvc.perform(post("/api/catering/dispatchFoodPackage")
        .contentType(MediaType.APPLICATION_JSON)
        .content(jsonObject.toString()))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.id").exists())
  ...
  void getFoodPackages() throws Exception {
    FoodPackageDTO packageDTO = new FoodPackageDTO(
      foodPackageId.toString(),
      recipeId.toString(),
      clientId.toString(),
      addressId.toString(),
    ...
    mockMvc.perform(get("/api/catering/getAllPackages")
        .contentType(MediaType.APPLICATION_JSON))
      .andExpect(status().isOk())
